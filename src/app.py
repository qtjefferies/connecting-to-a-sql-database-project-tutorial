...
def connect():
    ...
    connection_string = f"postgresql://{os.getenv('sqldb')}:{os.getenv('postgres')}@{os.getenv('localhost')}/{os.getenv('sample-db')}"
...
# 2) Execute the SQL sentences to create your tables using the SQL
engine.execute('''
